...
function ResourceParse() {
  ...
  if (type0 == "Subs-B64Encode") {
    ...
    total = Rewrite_Filter(
      ...
      Preg,
      Pregout
    ); //Rewrite_Filter(total, Pin0, Pout0,Preg,Pregout); // 筛选过滤
    if (Preplace) {
  ...
  if (flag == 1) {
    ...
    if (total.length > 0) {
      ...
      total =
        para1.indexOf("node_index_prefix") != -1 ? index_handle(total) : total; // 节点序号操作
...
function Type_Check(subs) {
  ...
  const NodeCheck2 = (item) =>
    subi.toLowerCase().indexOf(item.toLowerCase()) != -1; //URI 类型
  ...
  if (
    ...
    typec = "rewrite";
    type =
      typeQ == "unsupported" || typeQ == "rewrite" ? "rewrite" : "wrong-field"; //Quantumult X 类型 rewrite/ Surge Script/
  } else if (
    ...
    type =
      typeQ == "unsupported" || typeQ == "server"
        ...
        : "wrong-field";
    if (content0.split("\n").length >= 2) {
      //  local snippet and first line remarks
...
function Mock2QXReject(row, filename) {
  if (/dict/i.test(filename)) {
    return (
      row.replace(/ /g, "").split("data=")[0].split("data-type=")[0] +
      " url " +
      "reject-dict"
    );
  ...
  for (var i = 0; i < subs.length; i++) {
    try {
      ...
      if (!NoteK.some(notecheck) && !RewriteK.some(RewriteCheck)) {
        ...
        if (SC.every(sccheck)) {
        ...
        } else if (/\d\s30(7|2)$/.test(subs[i])) {
          //rewrite 302&307 复写(Surge)
          ...
          nrw.push(rw);
        } else if (/\d\s\-\s30(2|7)\s/.test(subs[i])) {
          //rewrite 302&307 复写(Shadowrocket)
          //xx - 302 $1$2$3
...
function Rule_Handle(subs, Pout, Pin) {
  ...
  ); //去除端口号以及分号部分, 以及部分suffix规则以. 开头的问题
  //$notify("nlist","",nlist)
  return nlist;
}
...
function Rule_Policy(content) {
  ...
  if (RuleK1.some(RuleCheck1) && !RuleK.some(RuleCheck)) {
    ...
    if (cnt[0].indexOf("URL-REGEX") != -1 || cnt[0].indexOf("PROCESS") != -1) {
      ...
      nn = nn
        .replace("IP-CIDR6", "ip6-cidr")
        .replace(/^(domain|Domain|DOMAIN)/, "host");
    }
    return nn;
  } else if (
    cnt.length == 1 &&
...
function rule_list_handle(cnt) {
  ...
  if (
    ...
    if (cnt.indexOf("::") != -1 && cnt.indexOf("/") != -1) {
      ...
      //host - suffix, not clash rule list
      //$notify("xxx","xxxx",cnt)
    ...
    if (
      ...
      try {
        ...
        if (
          ...
          if (listi.indexOf("@") != -1) {
            ...
            listh = list0[i].replace(
              listi
                ...
                .split("?")[0],
              listh
            ); //type+"://" + listh + "#" + listi.split(type+"://")[1].split("#")[1]
          ...
          Perror = 1; // 无需反馈
          $notify(
            "⚠️ 你的 Quantumult X 版本暂未支持 Vless 节点",
            "请 ⚠️不要⚠️ 跑来 解析器🤖️ 反馈",
            list0[i]
          );
        } else if (type == "vless") {
  ...
  if (server != "") {
    if (server.indexOf("@") != -1) {
      ...
      var ipport =
        server.split("#")[0].indexOf(":") == -1
          ...
          : "http=" + server.split("#")[0].split("?")[0]; // https://b64(ipport)
    }
    ...
    var thost =
      subs.indexOf("peer=") != -1
        ...
        : ""; // 存在peers参数时 https://b64(ipport)?peer=xxx#server-remarks
...
function SSR2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 8) {
    ...
    ssrp =
      cnt.split(":")[2] != "origin" ? "ssr-protocol=" + cnt.split(":")[2] : ""; //origin?
...
function VL2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  obfs = "";
  thost = "";
  if (cnt.indexOf("remarks=") == -1 && cnt.indexOf("@") != -1) {
    // normal URI
    typeU = "URI";
    ...
    typeU = "SR-URI";
    tag =
      cnt.indexOf("remarks=") != -1
        ? "tag=" + decodeURIComponent(cnt.split("remarks=")[1].split("&")[0])
        : "tag= [vless]" + ip;
  ...
  if (typeU == "SR-URI") {
    ...
    puri =
      cnt.indexOf("path=") == -1
        ? puri
        : "obfs-uri=" +
          decodeURIComponent(cnt.split("path=")[1].split("&")[0].split("#")[0]);
  } else if (
  ...
  } else if (cnt.indexOf("security=xtls") != -1) {
    //暂不支持类型
    type = "NS";
...
function SS2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 10) {
    ...
    if (cntt.indexOf("@") != -1 && cntt.indexOf(":") != -1) {
      ...
      if (cntt.indexOf("%") == -1 || cntt.split("@")[0].indexOf(":") == -1) {
        // 2025-05-16
...
function QXFix(cntf) {
  ...
  try {
    ...
    cntis = cnti
      .split(",")
      .filter(Boolean)
      .map((item) => item.trim()); //防止节点名中有,符号而导致的错误情况
    tagfix = "";
    cntii = "";
    for (i in cntis) {
...
function isQuanXRewrite(content) {
  ...
  for (var i = 0; i < cnt.length; i++) {
    if (cnt[i]) {
      ...
      } else if (Pjsonjq == 0 && cnti.indexOf(" url jsonjq-") != -1) {
        // lower version jsonjq pass
        cnti = "";
      } else {
        cnti = "";
      }
...
function SSS2QX(content) {
  ...
  var pwd =
    "password=" + cnts.split("password=")[1].split(",")[0].replace('"', ""); //.split("\"")[0];
  if (cnt.indexOf("obfs") != -1) {
    pobfs =
      "obfs=" +
      cnt
...
function LoonVL2QX(cnt) {
  ...
  vpath =
    cnt.indexOf("path=") == -1
      ? ""
      : "obfs-uri=" + cnt.split("path=")[1].split(",")[0];
