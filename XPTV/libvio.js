...
const appConfig = {
  ver: 1,
  title: "LIBVIO",
  site: "https://libvio.host/", // 改这里：从 libvio.cc 改成 libvio.la
  tabs: [
  ...
  if (groups.length === 0) {
    ...
    if (playBtn) {
      groups.push({
        title: "立即播放",
        tracks: [
          { name: "第1集", pan: "", ext: { url: appConfig.site + playBtn } },
        ],
      });
    }
  }
  ...
  $("div.netdisk-panel, div.playlist-panel.netdisk-panel").each((_, panel) => {
    ...
    $panel.find(".netdisk-list a").each((_, item) => {
      ...
      groups.push({
        ...
        tracks: [
          {
            name: a.find(".netdisk-name").text().trim() || "合集",
            pan: a.attr("href"),
          },
        ],
      });
    ...
    try {
      ...
      if (match) {
        ...
        if (obj.encrypt === "1") {
          ...
          playerUrl = unescape(
            CryptoJS.enc.Base64.parse(playerUrl).toString(CryptoJS.enc.Utf8),
          );
        } else if (obj.encrypt === "3") {
          playerUrl = CryptoJS.enc.Base64.parse(playerUrl).toString(
            CryptoJS.enc.Utf8,
          );
        }
        ...
        if (playerUrl.startsWith("http")) {
          ...
          const parseUrlMatch = tyData.match(
            /var\s+PARSE_URL\s*=\s*['"]([^'"]+)['"]/,
          );
          ...
          for (let attempt = 0; attempt < MAX_RETRY; attempt++) {
            try {
              const { data: parseData } = await $fetch.post(
                appConfig.site + parseUrl,
                postBody,
                {
                  headers: {
                    Referer: ty_new,
